...
WORKDIR /wsi-service/wsi_service_base_plugins/cooler
RUN poetry build && poetry export -f requirements.txt > requirements.txt
...
COPY --from=wsi_service_build /wsi-service/wsi_service_base_plugins/cooler/requirements.txt /artifacts/requirements_cooler.txt
RUN pip install -r /artifacts/requirements_cooler.txt
...
COPY --from=wsi_service_build /wsi-service/wsi_service_base_plugins/cooler/dist/ /wsi-service/dist/
